;;; -*-emacs-lisp-*-
;;; doxymacs-extended.el --- ELisp package for making doxygen related stuff easier.
;;
;; Modification of doxymacs.el to be able to generate doxygen
;; documentation in Morphee's style :)
;; modified by Thibauld Nion, 06/11/2006
;;
...
;;
;; $Id: doxymacs.el.in,v 1.23 2006/04/23 01:20:15 ryants Exp $

;; Commentary:
;;
...
(defconst doxymacs-version "1.7.0"
...
Must be one of \"JavaDoc\", \"Qt\" or \"C++\" or \"Morphee\". Setting this variable
to anything else will generate errors."
  :type '(radio (const :tag "JavaDoc" "JavaDoc")
		(const :tag "Qt" "Qt")
		(const :tag "C++" "C++")
		(const :tag "Morphee" "Morphee"))
  :group 'doxymacs)
...
\\{doxymacs-mode-map}"
  ...
  (when doxymacs-mode
    (when (boundp 'filladapt-token-table)
      ...
      (let ((bullet-regexp "[@\\]\\(param\\(?:\\[\\(?:in\\|out\\|in,out\\)\\]\\)?\\s-+\\sw+\\|return\\)"))
...
(defconst doxymacs-Morphee-file-comment-template
 '("/*!" > n
   ...
   " * " p > n
   "*/" > n)
 "Default Morphee-style template for file documentation.")
...
style."
  (error (concat
	  ...
	  doxymacs-doxygen-style
	  ": must be one of \"JavaDoc\", \"Qt\" or \"C++\" or \"Morphee\".")))
